...
dTable # jeœli tabela ma conajmniej 100 wierszy to wyœwietla 5 pierw. i 5 ost.
class(dTable) # jest rozszerzeniem data.frame
...
address(dTable)
names(dTable) <- c("sLength", "sWidth", "pLength", "pWidth",
  "species") # warninga nie ma 
...
#                    (na jakiejœ skali)
cases[, .(
  MinPop=min(PopulationInCiti, na.rm=T),
  AvgPop=mean(PopulationInCiti, na.rm=T),
  MaxPop=max(PopulationInCiti, na.rm=T)), by=Land][order(Land)]
...
x # jest x
chV = "x" # nazwa kolumny
get(chV) # dobranie sie do wartoœci kryj¹cecej siê w zmiennej o danej nazwie

library(Rcpp) # kolejny sposób na przyspieszenie obliczeñ (potrzebne RTools3.4)
